...
class _SystemConfigViewState extends State<_SystemConfigView>
  ...
  Widget build(BuildContext context) {
    ...
    final isHrOrAdmin =
        isSuperAdmin ||
        sessionState.role == UserRole.hrAdmin ||
        sessionState.role == UserRole.hr;
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                ...List.generate(
                  tabs.length,
                  (i) => _buildTabItem(tabs[i], i, i == _selectedIndex),
                ),
              ],
            ),
          ),
          Expanded(
            child: SlideTransition(
              ...
              child: FadeTransition(
                ...
                child: _buildContent(isSuperAdmin, isHrOrAdmin),
              ),
            ),
          ),
        ],
  ...
  Widget _buildContent(bool isSuperAdmin, bool isHrOrAdmin) {
    final allContent = <_ContentDef>[
      _ContentDef(
        'Leave Types',
        Iconsax.calendar_2,
        (ctx) => const LeaveTypesContent(),
      ),
      _ContentDef('Holidays', Iconsax.sun, (ctx) => const HolidaysContent()),
      _ContentDef(
        'Departments',
        Iconsax.buildings,
        (ctx) => const DepartmentsContent(),
      ),
      if (isSuperAdmin)
        _ContentDef(
          'Roles & Permissions',
          Iconsax.lock,
          (ctx) => const RolesContent(),
        ),
      if (isSuperAdmin)
        _ContentDef(
          'Company Settings',
          Iconsax.building,
          (ctx) => const CompanyContent(),
        ),
      if (isHrOrAdmin)
        _ContentDef(
          AppLocals.officeBranches.tr(),
          Iconsax.map,
          (ctx) => const BranchesContent(),
        ),
      if (isHrOrAdmin)
        _ContentDef(
          'Employee Branches',
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(
                allContent[_selectedIndex].icon,
                size: 22,
                color: AppColors.primary,
              ),
              const SizedBox(width: 12),
              Text(
                allContent[_selectedIndex].title,
