...
import 'executive_metric_card.dart';

class ExecutiveBody extends StatefulWidget {
  final ExecutiveDashboardData data;
  const ExecutiveBody({super.key, required this.data});

  @override
  State<ExecutiveBody> createState() => ExecutiveBodyState();
}

class ExecutiveBodyState extends State<ExecutiveBody> with SingleTickerProviderStateMixin {
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              MetricCard(label: 'Total Headcount', finalValue: summary.totalEmployees.toDouble(), unit: '', trend: '+3%', positive: true, color: const Color(0xFF3B82F6), icon: Iconsax.people, animation: _animation),
              const SizedBox(width: 16),
              MetricCard(label: 'Present Today', finalValue: summary.presentPercent * 100, unit: '%', trend: '+2%', positive: true, color: const Color(0xFF22C55E), icon: Iconsax.verify, animation: _animation),
              const SizedBox(width: 16),
              MetricCard(label: 'Avg KPI Score', finalValue: summary.avgKpiScorePercent * 100, unit: '', trend: '+1.2', positive: true, color: const Color(0xFFF59E0B), icon: Iconsax.chart_2, animation: _animation),
              const SizedBox(width: 16),
              MetricCard(label: 'Engagement', finalValue: summary.engagementScorePercent * 100, unit: '%', trend: '+5%', positive: true, color: const Color(0xFF8B5CF6), icon: Iconsax.heart, animation: _animation),
            ],
          ),
