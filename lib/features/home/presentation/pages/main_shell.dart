...
class MainShell extends StatelessWidget {
    ...
    return SafeArea(
      child: Scaffold(
        ...
        bottomNavigationBar: BottomNavigationBar(
          ...
          currentIndex: _calculateSelectedIndex(context, showTeamTab),
          onTap: (index) => _onItemTapped(index, context, showTeamTab),
          items: items,
        ),
      ),
    );
  ...
  static int _calculateSelectedIndex(BuildContext context, bool showTeamTab) {
    ...
    if (location.startsWith('/profile')) return showTeamTab ? 5 : 4;
    return 0;
  }

  void _onItemTapped(int index, BuildContext context, bool showTeamTab) {
    if (index == 0) {
      context.go('/home');
    } else if (index == 1) {
      ...
      context.go('/team');
    } else if ((showTeamTab && index == 5) || (!showTeamTab && index == 4)) {
      context.go('/profile');
    }
  }
}
